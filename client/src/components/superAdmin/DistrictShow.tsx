  ...
  if (isLoading) {
    return (
      <div className="space-y-5">
        ...
        <Card className="overflow-hidden border bg-card shadow-none" aria-label="Loading districts">
    ...
    <div className="animate-enter space-y-5">
      ...
      <Card className="border bg-card shadow-none">
        <CardContent className="flex flex-col items-center justify-center px-5 py-14 text-center">
          ...
          <h2 className="section-title">Failed to load districts</h2>
    ...
    <div className="animate-enter space-y-5">
      ...
      {districts.length === 0 ? (
        <Card className="border bg-card shadow-none">
          <CardContent className="flex flex-col items-center justify-center px-5 py-14 text-center">
            ...
            <h2 className="section-title">No districts yet</h2>
            <p className="mb-5 mt-1 max-w-sm text-sm text-muted-foreground">
              Create the first district from the Create Items page.
            </p>
              ...
              {districts.map((district, index) => (
                <li key={district._id} className="flex items-center gap-3 px-4 py-2.5 sm:px-5">
                  ...
                  <span className="ml-auto flex shrink-0 gap-1.5">
                    <Button size="sm" variant="outline" onClick={() => { setCurrent({ _id: district._id, name: district.name }); setIsDialogOpen(true); }} aria-label={`Edit ${district.name}`}>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[480px]">
          ...
          >
            <DialogHeader>
              <DialogTitle className="section-title">Update District</DialogTitle>
              <DialogDescription>Update the record name.</DialogDescription>
            </DialogHeader>
            <div className="space-y-1.5">
              <Label htmlFor="edit-district-name">Name</Label>
              <Input
