...
const UnionShow = () => {
  ...
  const [updateUnion, { isLoading: isUpdating }] = useUpdateUnionMutation();
  ...
  if (isLoading) {
    return (
      <div className="space-y-5">
        ...
        <Card className="overflow-hidden border bg-card shadow-none" aria-label="Loading unions">
    ...
    <div className="animate-enter space-y-5">
      ...
      <Card className="border bg-card shadow-none">
        <CardContent className="flex flex-col items-center justify-center px-5 py-14 text-center">
          ...
          <h2 className="section-title">Failed to load unions</h2>
    ...
    <div className="animate-enter space-y-5">
      ...
      {unions.length === 0 ? (
        <Card className="border bg-card shadow-none">
          <CardContent className="flex flex-col items-center justify-center px-5 py-14 text-center">
            ...
            <h2 className="section-title">No unions yet</h2>
            <p className="mb-5 mt-1 max-w-sm text-sm text-muted-foreground">
              Create the first union from the Create Items page.
            </p>
              ...
              {pageItems.map((union: { _id: string; name: string }, index: number) => (
                <li key={union._id} className="flex items-center gap-3 px-4 py-2.5 sm:px-5">
                  ...
                  <span className="ml-auto flex shrink-0 gap-1.5">
                    <Button size="sm" variant="outline" onClick={() => { setCurrent(union); setIsDialogOpen(true); }} aria-label={`Edit ${union.name}`}>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[480px]">
          ...
          >
            <DialogHeader>
              <DialogTitle className="section-title">Update Union</DialogTitle>
              <DialogDescription>Update the record name.</DialogDescription>
            </DialogHeader>
            <div className="space-y-1.5">
              <Label htmlFor="edit-union-name">Name</Label>
              <Input
                id="edit-union-name"
                className="h-10"
                value={current?.name || ""}
