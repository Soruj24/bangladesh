...
import { AlertTriangle, Loader2, Pencil, Trash2, TreePine } from "lucide-react";
  ...
  if (isLoading) {
    return (
      <div className="space-y-5">
        ...
        <Card className="overflow-hidden border bg-card shadow-none" aria-label="Loading villages">
    ...
    <div className="space-y-5">
      ...
      <Card className="border bg-card shadow-none">
        <CardContent className="flex flex-col items-center justify-center px-5 py-14 text-center">
          ...
          <h2 className="section-title">Failed to load villages</h2>
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-end justify-between gap-2">
        <div>
          ...
          <p className="page-sub tabular" aria-live="polite">
            {(villageData?.villagesWithOutUnion?.length ?? 0)} records
          </p>
        </div>
      </div>
      ...
      {!villageData?.villagesWithOutUnion?.length ? (
        <Card className="border bg-card shadow-none">
          <CardContent className="flex flex-col items-center justify-center px-5 py-14 text-center">
            <div className="mb-4 rounded-full bg-muted p-3.5">
              <TreePine className="h-6 w-6 text-muted-foreground" aria-hidden="true" />
            </div>
            <h2 className="section-title">No villages yet</h2>
            <p className="mb-5 mt-1 max-w-sm text-sm text-muted-foreground">
              Create the first village from the Create Items page.
            </p>
        ...
        <Card className="overflow-hidden border bg-card shadow-none">
          <CardContent className="p-0">
            <ul className="divide-y divide-border">
              {villageData?.villagesWithOutUnion?.map((village, index) => (
                <li key={village._id} className="flex items-center gap-3 px-4 py-2.5 sm:px-5">
                  ...
                  <span className="ml-auto flex shrink-0 gap-1.5">
                    <Button size="sm" variant="outline" onClick={() => { setCurrent(village); setIsDialogOpen(true); }} aria-label={`Edit ${village.name}`}>
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[480px]">
          ...
          >
            <DialogHeader>
              <DialogTitle className="section-title">Update Village</DialogTitle>
              <DialogDescription>Update the record name.</DialogDescription>
            </DialogHeader>
            <div className="space-y-1.5">
              <Label htmlFor="edit-village-name">Name</Label>
              <Input
