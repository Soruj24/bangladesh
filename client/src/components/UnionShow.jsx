import { 
  useDeleteUnionMutation, useHandelGetUnionsQuery, useUpdateUnionMutation } from "../services/unionApi"

import { useState } from 'react';
import { toast, ToastContainer } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
const DivisionShow = () => {
  const { data, isLoading, error, refetch } = useHandelGetUnionsQuery();
  ...
  const handleDelete = async (id) => {
    try {
      ...
      console.error(err);
      toast.error("Failed to delete union. Please try again.");
    }
  };

  // Handle edit action
  const handleEditClick = (union) => {
    setEditingDivisionId(union._id);
    setUpdatedName(union.name);
  };

  // Handle update action
  const handleUpdate = async (id) => {
    try {
      await updateUnion({ id, name: updatedName }).unwrap();
          ...
          data?.union.map((union, index) => (
            ...
            >
              {editingDivisionId === union._id ? (
                ...
                <p className="text-gray-700 font-medium">{index + 1}. {union.name}</p>
              )}
              <div className="flex gap-2">
                {editingDivisionId === union._id ? (
                  <button
                    onClick={() => handleUpdate(union._id)}
