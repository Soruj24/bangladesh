...
import { useDeleteDivisionMutation, useGetDivisionsQuery, useUpdateDivisionMutation } from "../services/divisionApi";
import { toast, ToastContainer } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
const DivisionShow = () => {
    const { data, isLoading, error, refetch } = useGetDivisionsQuery();
    ...
    const handleUpdate = async (id) => {
        try {
            await updateDivision({ id, name: updatedName }).unwrap();
                ...
                data.divisions.map((division, index) => (
                    ...
                    >
                        {editingDivisionId === division._id ? (
                            ...
                            <p className="text-gray-700 font-medium">{index + 1}. {division.name}</p>
                        )}
                        <div className="flex gap-2">
                            {editingDivisionId === division._id ? (
                                <button
                                    onClick={() => handleUpdate(division._id)}
