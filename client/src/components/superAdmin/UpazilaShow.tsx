...
import { AlertTriangle, Landmark, Loader2, Pencil, SearchX, Trash2 } from "lucide-react";
import { Link } from "react-router-dom";

const UpazilaShow = () => {
  const { data, isError, isLoading, refetch } = useGetAllUpazilasQuery();
  const [updateUpozila, { isLoading: isUpdating }] = useUpdateUpozilaMutation();
  ...
  if (isLoading) {
    return (
      <div className="space-y-5">
        ...
        <Card className="overflow-hidden border bg-card shadow-none" aria-label="Loading upazilas">
    ...
    <div className="animate-enter space-y-5">
      ...
      <Card className="border bg-card shadow-none">
        <CardContent className="flex flex-col items-center justify-center px-5 py-14 text-center">
          ...
          <h2 className="section-title">Failed to load upazilas</h2>
    ...
    <div className="animate-enter space-y-5">
      ...
      {upazilas.length === 0 ? (
        <Card className="border bg-card shadow-none">
          <CardContent className="flex flex-col items-center justify-center px-5 py-14 text-center">
            ...
            <h2 className="section-title">No upazilas yet</h2>
            <p className="mb-5 mt-1 max-w-sm text-sm text-muted-foreground">
              Create the first upazila from the Create Items page.
            </p>
        ...
        <Card className="overflow-hidden border bg-card shadow-none">
          <CardContent className="p-0">
            <ul className="divide-y divide-border">
              {pageItems.map((upazila, index) => (
                <li key={upazila._id || `upazila-${index}`} className="flex items-center gap-3 px-4 py-2.5 sm:px-5">
                  ...
                  <span className="ml-auto flex shrink-0 gap-1.5">
                    <Button size="sm" variant="outline" onClick={() => { setCurrent({ _id: upazila._id || '', name: upazila.name }); setIsDialogOpen(true); }} aria-label={`Edit ${upazila.name}`}>
                    ...
                    <Button
                      ...
                      aria-label={`Delete ${upazila.name}`}
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="sm:max-w-[480px]">
          ...
          >
            <DialogHeader>
              <DialogTitle className="section-title">Update Upazila</DialogTitle>
              <DialogDescription>Update the record name.</DialogDescription>
            </DialogHeader>
            <div className="space-y-1.5">
              <Label htmlFor="edit-upazila-name">Name</Label>
              <Input
                id="edit-upazila-name"
                className="h-10"
                value={current?.name || ""}
                onChange={(e) => setCurrent(prev => prev ? { ...prev, name: e.target.value } : null)}
              />
            </div>
