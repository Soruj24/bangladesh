...
import { useDeleteDistrictMutation, useGetDistrictsQuery, useUpdateDistrictMutation } from "@/services/districtApi"

const DistrictShow = () => {
    const { data, isError, isLoading, refetch } = useGetDistrictsQuery()
    const [updateDistrict] = useDeleteDistrictMutation()
    const [deleteDistrict] = useUpdateDistrictMutation()
    ...
    const handleDelete = async (id: number) => {
        const res = await updateDistrict(id)
        console.log(res)
    ...
    const handleUpdateSubmit = async (updatedDivision: any) => {
        ...
        try {
            const res = await deleteDistrict({ ...updatedDivision, id: updatedDivision._id }).unwrap();
    ...
    if (isLoading) {
        return (
            <div className="space-y-6 sm:grid sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                ...
                {[...Array(data?.districts.length || 1)].map((_, index) => (
